...
- (void)setupUI {
    ...
    self.userInfoLabel.text = [NSString stringWithFormat:@"UserID:%@  UserName:%@", self.localUserID, self.localUserID];
...
- (void)rearrangeVideoTalkViewObjects {
    ...
    CGFloat contentHeight = MAX((ZGVideoTalkStreamViewHeight + viewSpacing) * rowCount, self.containerView.bounds.size.height);
...
- (void)updateStreamPopupViewInfo {
    ...
    [self.streamPopupView updateWithTitle:@"User List" textList:allStreamInfoList];
}
...
- (ZGVideoTalkViewObject *)localUserViewObject {
    if (!_localUserViewObject) {
        ...
        _localUserViewObject.userName = _localUserID;
