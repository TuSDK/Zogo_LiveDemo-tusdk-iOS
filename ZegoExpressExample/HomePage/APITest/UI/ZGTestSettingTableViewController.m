...
- (IBAction)createEngineClick:(UIButton *)sender {
    [self.manager createEngineWithAppID:[self.appIDTextField.text intValue] scenario:(ZegoScenario)self.scenarioSeg.selectedSegmentIndex];
}
...
- (IBAction)loginRoomClick:(UIButton *)sender {
    [self.manager loginRoom:self.roomIDTextField.text userID:self.userIDTextField.text userName:self.userNameTextField.text token: self.tokenTextField.text];
}
