...
- (void)captureDevice:(id<ZGCaptureDevice>)device didCapturedData:(CMSampleBufferRef)data {
    ...
    if (self.captureBufferType == ZGCustomVideoCaptureBufferTypeCVPixelBuffer) {
        ...
        CMTime presentationTimeStamp = CMSampleBufferGetPresentationTimeStamp(data);
        int64_t timeStampMs = (1000 * presentationTimeStamp.value) / presentationTimeStamp.timescale;
        
        CVPixelBufferRef newPixelBuffer = [[TuSDKManager sharedManager] syncProcessPixelBuffer:CMSampleBufferGetImageBuffer(data) timeStamp:timeStampMs];
                
        // Send pixel buffer to ZEGO SDK
        [[ZegoExpressEngine sharedEngine] sendCustomVideoCapturePixelBuffer:newPixelBuffer timestamp:CMSampleBufferGetPresentationTimeStamp(data)];
